...
export default async function Page({ searchParams }: { searchParams: Promise<{ uploaded?: string;deleted?:string;error?:string;vendor?:string;project?:string;category?:string;file?:string }> }) {
  ...
  const [r,vr,pr] = await Promise.all([
    s.from("documents").select("id,document_title,document_type,document_category,file_name,revision,purchase_order_id,invoice_id,vendor_id,project_id,google_drive_file_id,is_historical,uploaded_at,storage_provider,storage_status,file_size,is_deleted").eq("is_deleted", false).order("uploaded_at", { ascending: false }),
