...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string}>}){
  ...
  const upcoming=milestones.filter((m:any)=>m.payment_due_date).sort((a:any,b:any)=>String(a.payment_due_date).localeCompare(String(b.payment_due_date))).slice(0,6);
  ...
  const nextDue=upcoming[0]?.payment_due_date||null;
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    {selectedVendor?<section className="mb-5 rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-white p-5 dark:border-blue-900/40 dark:from-blue-950/30 dark:to-slate-950">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MiniMetric title="Next Payment Due" icon={CalendarClock} value={nextDue?formatDate(nextDue):"No date"} helper={nextDue?"Next active milestone date":"No scheduled milestone"}/>
      </div>
    </section>:null}
