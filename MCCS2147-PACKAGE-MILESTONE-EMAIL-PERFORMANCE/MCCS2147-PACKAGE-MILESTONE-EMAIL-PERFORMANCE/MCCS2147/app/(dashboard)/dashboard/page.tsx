...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string;month?:string;day?:string}>}){
  ...
  const packageBreakdown=selectedPackages.map((pkg:any)=>{
    const packagePos=pos.filter((po:any)=>String(po.project_id||"")===String(pkg.id));
    const packagePoIds=new Set(packagePos.map((po:any)=>String(po.id)));
    const packageInvoices=invoices.filter((inv:any)=>packagePoIds.has(String(inv.purchase_order_id)));
    const packagePayments=payments.filter((pay:any)=>packagePoIds.has(String(pay.purchase_order_id)));
    const packageOutstanding=outstandingSorted.filter((m:any)=>packagePoIds.has(String(m.purchase_order_id)));
    ...
    return {
      ...
      totalMilestoneCount:milestones.filter((m:any)=>packagePoIds.has(String(m.purchase_order_id))).length,
