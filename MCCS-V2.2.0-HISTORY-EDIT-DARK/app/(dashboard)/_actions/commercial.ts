...
export async function createInvoice(formData: FormData) {
  ...
  revalidatePath("/invoices"); revalidatePath("/dashboard"); redirect("/invoices?created=1");
}
...
export async function createDocumentMetadata(formData: FormData){
 const {supabase,user}=await requireSuperAdmin();const {error}=await supabase.from("documents").insert({vendor_id:nullable(formData,"vendor_id"),purchase_order_id:nullable(formData,"purchase_order_id"),milestone_id:nullable(formData,"milestone_id"),document_type:text(formData,"document_type"),file_name:text(formData,"file_name"),revision:nullable(formData,"revision"),onedrive_path:nullable(formData,"onedrive_path"),is_historical:formData.get("is_historical")==="on",uploaded_by:user.id});
