...
export async function GET() {
  ...
  const [poR,vR,mR,iR,pR] = await Promise.all([
    ...
    supabase.from("invoices").select("invoice_number,invoice_date,invoice_amount,certified_amount,status,due_date"),
    supabase.from("payments").select("payment_date,paid_amount,payment_reference,bank_reference"),
  ]);
