...
import { ensureVendorDriveFolder, ensureProjectDriveStructure, ensurePOFolder, ensureInvoiceFolder, uploadFileToGoogleDrive, sanitizeDriveName, ensureGoogleFolderPath, moveGoogleDriveFile, updateGoogleDriveFile } from "@/lib/google/drive";
...
async function syncVdrlControlledOutput(supabase:any, registerId:string, userId:string) {
  ...
  try {
    ...
    ws.mergeCells("J3:K3"); ws.getCell("J3").value="IFR";
    ws.mergeCells("L3:M3"); ws.getCell("L3").value="IFA";
    ws.mergeCells("N3:O3"); ws.getCell("N3").value="IFC";
    const headers=["Document Number","Document Title","Supplier","Contractor","Package","Discipline","Sub-Discipline","Document Type","Stage","Planned Submit","Actual Submit","Planned Return","Actual Return","Planned Submit","Actual Submit","Revision","Return Code","Status","Resubmission Due","Remarks"];
    ...
    for (const d of (docs??[])) ws.addRow([
      d.document_number,d.document_title,supplierName,contractorName,packageName,d.discipline,d.sub_discipline,d.document_type,d.current_stage,
      ...
      d.current_revision,d.return_code,niceVdrlStatus(d.current_status),d.resubmission_due_date,d.comments
    ]);
    ...
    [10,12,14].forEach(c=>{const cell=ws.getCell(3,c);cell.font={bold:true,color:{argb:"FF071C35"}};cell.fill={type:"pattern",pattern:"solid",fgColor:{argb:`FF${green}`}};cell.alignment={horizontal:"center"};});
    ...
    for (let r=5;r<=ws.rowCount;r++) { [10,11,12,13,14,15,19].forEach(c=>ws.getCell(r,c).numFmt="dd-mmm-yyyy"); }
...
export async function uploadChatVoice(formData: FormData) {
  ...
  const { data: message, error } = await access.supabase.from("chat_messages").insert({
  ...
  }).select("id,sender_id,recipient_id,body,created_at,read_at,attachment_document_id,message_type,voice_duration_seconds,voice_waveform").single();
