...
export default async function Page({searchParams}:{searchParams:Promise<{deleted?:string;error?:string}>}){
  ...
  const r=await s.from("payments")
    .select("id,purchase_order_id,invoice_id,currency_id,payment_date,paid_amount,payment_reference,bank_reference,is_historical,is_deleted")
    .eq("is_deleted",false)
  ...
  const [pr,cr,pjr,ir]=await Promise.all([
    poIds.length?s.from("purchase_orders").select("id,po_number,vendor_id,project_id").in("id",poIds):Promise.resolve({data:[]} as any),
  ...
  const poProject=new Map<string,string>((pr.data??[]).map((x:any)=>[String(x.id),String(x.project_id||"")]));
  ...
  const im=new Map<string,string>((ir.data??[]).map((x:any)=>[String(x.id),String(x.invoice_number||"—")]));
  const pjm=new Map<string,string>((pjr.data??[]).map((x:any)=>[String(x.id),String(x.project_name||x.project_code||"Unnamed Package")]));
        ...
        <table className="w-full min-w-[1180px] text-left text-sm">
          ...
          <tbody className="divide-y dark:divide-slate-800">
            {rows.map((x:any)=>{
              ...
              const projectId=poProject.get(String(x.purchase_order_id))||"";
              return <tr key={x.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-900/50">
                ...
                <td className="px-5 py-4 font-bold text-blue-700">{x.invoice_id?im.get(String(x.invoice_id))||"—":"—"}</td>
                <td className="px-5 py-4 font-semibold text-slate-700 dark:text-slate-200">{pjm.get(projectId)||"—"}</td>
            ...
            {!rows.length?<tr><td colSpan={10} className="px-5 py-12 text-center text-slate-500">No payments recorded yet.</td></tr>:null}
          </tbody>
        </table>
      </div>
    </div>
