...
export default async function DashboardLayout({
  ...
  if (bootstrapSuperAdmin) {
    ...
    const [{ data: profileData }, { data: userRoles }, { data: permissionData }] = await Promise.all([
      supabase
        .from("profiles")
        .select("full_name, preferred_name, gender, honorific, is_super_admin, avatar_url")
        .eq("id", user.id)
    ...
    if (!profile?.is_super_admin) {
      ...
      if (roleIds.length) {
        const { data: roles } = await supabase.from("roles").select("role_code").in("id", roleIds);
        roleCodes = new Set((roles ?? []).map((r: any) => String(r.role_code).toLowerCase()));
      }
    }
  }
