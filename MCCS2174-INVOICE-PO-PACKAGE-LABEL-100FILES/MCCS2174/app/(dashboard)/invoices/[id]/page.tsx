...
import { assignInvoiceReviewer, reviewInvoice, finalApproveInvoice, returnInvoiceCommercial, markInvoiceSubmittedToAP, deleteInvoice } from "../../_actions/commercial";
...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  if (!invoice || invoice.is_deleted) return <div>Invoice not found.</div>;

  const [approvalResult, poResult, vendorResult, currencyResult, milestoneResult, profileResult, userRoleResult, roleResult, documentResult] = await Promise.all([
    ...
    s.from("documents").select("id,document_title,document_type,document_category,file_name,revision,file_size,uploaded_at").eq("invoice_id",id).eq("is_deleted",false).order("uploaded_at",{ascending:false}),
  ]);
  ...
  const linkedDocuments = documentResult.data ?? [];
  ...
  return <div className="mx-auto max-w-[1450px]">
    <div className="flex flex-wrap items-end justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2"><Link href={`/documents/upload?invoice=${id}`} className="rounded-xl bg-[#07111f] px-4 py-3 text-sm font-bold text-white dark:bg-blue-600">Attach Document</Link><Link href={`/invoices/${id}/verification`} className="rounded-xl border bg-white px-4 py-3 text-sm font-bold dark:bg-slate-900">Verification Sheet</Link><Link href={`/invoices/${id}/edit`} className="rounded-xl border bg-white px-4 py-3 text-sm font-bold dark:bg-slate-900">Edit Invoice</Link><Link href="/invoices" className="rounded-xl border bg-white px-4 py-3 text-sm font-bold dark:bg-slate-900">Back</Link></div>
    </div>
    {q.error ? <div className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{q.error}</div> : null}
    <section className="mccs-card mt-6 rounded-2xl p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-bold">Invoice Attachments</h2><p className="mt-1 text-sm text-slate-500">Files attached to this invoice in MCCS / Google Drive.</p></div><Link href={`/documents/upload?invoice=${id}`} className="rounded-xl border bg-white px-4 py-2.5 text-sm font-bold text-blue-700">Attach another file</Link></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{linkedDocuments.map((d:any)=><a key={d.id} href={`/api/documents/${d.id}/file`} target="_blank" rel="noreferrer" className="rounded-xl border p-4 transition hover:border-blue-300 hover:bg-blue-50/50"><div className="font-bold text-blue-700">{d.file_name||d.document_title||"Attached file"}</div><div className="mt-1 text-xs text-slate-500">{d.document_type||"Document"}{d.revision?` • ${d.revision}`:""}</div><div className="mt-3 text-sm font-bold">View / Download</div></a>)}{!linkedDocuments.length?<div className="rounded-xl border border-dashed p-4 text-sm text-slate-500">No invoice file is attached yet.</div>:null}</div></section>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <section className="mccs-card rounded-2xl p-6">
        <h2 className="text-lg font-bold">Invoice Controls</h2>
        <p className="mt-2 text-sm text-slate-500">
          Edit the invoice at any time. Delete is audit-safe: the invoice is removed from active registers but retained in the database history.
        </p>
        {a.isSuperAdmin ? (
          <form action={deleteInvoice} className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 dark:bg-red-950/20">
            <input type="hidden" name="invoice_id" value={id} />
            <label className="block text-sm font-bold text-red-800 dark:text-red-200">Reason for deletion</label>
            <input name="delete_reason" required className="input mt-2" placeholder="Reason for deleting / cancelling this invoice" />
            <button className="mt-3 w-full rounded-xl bg-red-700 px-4 py-3 text-sm font-bold text-white">
              Delete Invoice
            </button>
          </form>
        ) : null}
