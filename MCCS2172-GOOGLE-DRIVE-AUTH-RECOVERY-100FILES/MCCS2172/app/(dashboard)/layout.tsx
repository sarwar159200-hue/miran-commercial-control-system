...
export default async function DashboardLayout({
  ...
  // Fast path for the bootstrap Super Admin: the email itself is already the
  // trusted bootstrap rule, so do not wait for role/permission queries before
  // rendering the navigation. This makes the menu appear immediately after sign-in.
  let profile: any = null;
  let roleCodes = new Set<string>();
  ...
  if (bootstrapSuperAdmin) {
    ...
    // Fetch all navigation authority data in one parallel round-trip group.
    // Roles is a tiny reference table; loading it in parallel avoids the old
    // second database request after user_roles, which noticeably delayed login.
    const [{ data: profileData }, { data: userRoles }, { data: permissionData }, { data: allRoles }] = await Promise.all([
      supabase
        .from("profiles")
        .select("full_name, preferred_name, gender, honorific, is_super_admin, avatar_url")
        .eq("id", user.id)
      ...
      supabase.from("roles").select("id,role_code"),
    ]);

    profile = profileData;
    tabPermissions = permissionData ?? [];

    if (!profile?.is_super_admin) {
      const roleIds = new Set((userRoles ?? []).map((x: any) => String(x.role_id)).filter(Boolean));
      roleCodes = new Set(
        (allRoles ?? [])
          .filter((r: any) => roleIds.has(String(r.id)))
          .map((r: any) => String(r.role_code).toLowerCase())
      );
    }
  }
