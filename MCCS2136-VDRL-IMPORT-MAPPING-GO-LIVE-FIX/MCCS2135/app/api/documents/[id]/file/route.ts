...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (source === "vdrl") {
    ...
    const { data: row, error } = await supabase
      .from("documents")
      .select("id,file_name,google_drive_file_id,google_drive_web_url,storage_provider,is_deleted,document_category")
      .eq("id", id)
      .maybeSingle();
    if (error || !row || row.is_deleted) return new Response("Document not found.", { status: 404 });
    if (row.document_category === "chat_voice") {
      const { data: allowed } = await supabase
        .from("chat_messages")
        .select("id")
        .eq("attachment_document_id", id)
        .or(`sender_id.eq.${user.id},recipient_id.eq.${user.id}`)
        .limit(1);
      if (!allowed?.length) return new Response("You are not authorised to access this private chat voice message.", { status: 403 });
    }
    document = row;
  }
