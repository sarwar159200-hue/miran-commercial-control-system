            ...
            <tbody className="divide-y divide-slate-100">
              {vendors.map((v:any)=>(
                <tr key={v.id} className="bg-white">
                  ...
                  <td className="px-5 py-4"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">{v.is_active ? "Active" : "Inactive"}</span></td><td className="px-5 py-4"><div className="flex gap-3"><Link href={`/vendors/${v.id}/edit`} className="font-bold text-blue-700">Edit</Link>{isSuperAdmin?<form action={deleteVendorRecord}><input type="hidden" name="vendor_id" value={v.id}/><input type="hidden" name="delete_reason" value="Deleted from Vendor Register"/><button className="font-bold text-red-700">Delete</button></form>:null}</div></td>
                </tr>
              ))}
