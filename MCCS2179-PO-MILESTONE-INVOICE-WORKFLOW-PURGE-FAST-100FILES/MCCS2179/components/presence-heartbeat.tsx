...
export function PresenceHeartbeat({userId}:{userId?:string}){
  ...
  useEffect(()=>{let alive=true;void(async()=>{const uid=String(userId||"");if(!uid||!alive)return;setMe(uid);const [{data:p},{count}]=await Promise.all([supabase.from("profiles").select("id,full_name,preferred_name,avatar_url,last_seen_at").neq("id",uid).order("full_name"),supabase.from("chat_messages").select("id",{count:"exact",head:true}).eq("recipient_id",uid).is("read_at",null)]);if(!alive)return;const nextProfiles=(p||[]) as Profile[];profilesRef.current=nextProfiles;setProfiles(nextProfiles);setUnread(count||0);void supabase.rpc("touch_presence")})();return()=>{alive=false}},[supabase,userId]);
  ...
  useEffect(()=>{if(!me)return;
    ...
    const ch=supabase.channel(`mccs-global-comms-${me}`)
      ...
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"chat_calls",filter:`receiver_id=eq.${me}`},(p:any)=>{const c=p.new as Call;if(["calling","ringing"].includes(c.status)&&!activeCall&&!incomingRef.current&&callId.current!==c.id){incomingRef.current=c;setIncoming(c);partner.current=c.caller_id}})
