...
export async function updatePurchaseOrderDetails(formData: FormData){
 ...
 for(let i=0;i<milestoneCount;i++){
   ...
   const pctRaw=text(formData,`milestone_percentage_${i}`);
   const amountRaw=text(formData,`milestone_amount_${i}`);
   const pct=pctRaw?Number(pctRaw):null;
   if(pct!=null&&Number.isFinite(pct)) totalPct+=pct;
   rows.push({
     id:milestoneId,
     name,
     pct,
     amount:amountRaw?Number(amountRaw.replace(/,/g,"")):null,
     due:nullable(formData,`milestone_due_${i}`),
     paymentDue:nullable(formData,`milestone_payment_due_${i}`),
 ...
 // Existing rows can update in parallel; new rows are inserted in one request.
 // This is faster than one sequential database round trip per milestone.
 const updateResults=await Promise.all(existingRows.map((row:any)=>
   milestoneDb.from("payment_milestones").update({
     milestone_name:row.name,
     percentage:row.pct,
     fixed_amount:row.amount,
     planned_due_date:row.due,
     payment_due_date:row.paymentDue,
     status:row.status,
     sequence_no:row.sequence,
     updated_at:now,
     updated_by:user.id
   }).eq("id",row.id).eq("purchase_order_id",id).eq("is_deleted",false)
 ));
 const updateFailure=updateResults.find((r:any)=>r.error);
 if(updateFailure?.error) redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`Milestone update failed: ${updateFailure.error.message}`)}`);

 if(newRows.length){
   const payload=newRows.map((row:any)=>({
   ...
   const inserted=await milestoneDb.from("payment_milestones").insert(payload).select("id");
   if(inserted.error) redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`New milestone save failed: ${inserted.error.message}`)}`);
   if((inserted.data||[]).length!==payload.length) redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent("Milestone save did not return every new record. Please retry.")}`);
 }
