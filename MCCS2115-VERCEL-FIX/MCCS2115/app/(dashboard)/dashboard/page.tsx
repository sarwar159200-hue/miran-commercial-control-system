...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string;month?:string;day?:string;kpi?:string}>}){
  const q=await searchParams;
  const selectedVendor=String(q.vendor||"");
  const selectedKpi=["committed","certified","paid","remaining"].includes(String(q.kpi||""))?String(q.kpi):"";
  ...
  const vendorAgg=new Map<string,{vendorId:string;name:string;value:number}>();
  for(const p of pos){const vendorId=String(p.vendor_id||""),currency=cMap.get(String(p.currency_id))||"",key=`${vendorId}|${currency}`,name=`${vmap.get(vendorId)||"Unknown"} · ${currency}`;const x=vendorAgg.get(key)||{vendorId,name,value:0};x.value+=Number(p.current_value||0);vendorAgg.set(key,x);}
  const vendorData=[...vendorAgg.values()].sort((a,b)=>b.value-a.value);
  ...
  const cashflowData=[...months.values()].sort((a,b)=>a.month.localeCompare(b.month));
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><MoneyCard title="Committed" icon={ShoppingCart} rows={committed} helper={`${pos.length} active PO records`} href={`/dashboard?${kpiQuery(selectedVendor,calendarMonth,selectedDay,"committed")}`} active={selectedKpi==="committed"}/><MoneyCard title="Certified" icon={FileCheck2} rows={certified} helper={`${invoices.length} invoice records`} href={`/dashboard?${kpiQuery(selectedVendor,calendarMonth,selectedDay,"certified")}`} active={selectedKpi==="certified"}/><MoneyCard title="Paid" icon={Banknote} rows={paid} helper={`${payments.length} payment records`} href={`/dashboard?${kpiQuery(selectedVendor,calendarMonth,selectedDay,"paid")}`} active={selectedKpi==="paid"}/><MoneyCard title="Remaining" icon={ReceiptText} rows={remaining} helper="Committed less paid by currency" href={`/dashboard?${kpiQuery(selectedVendor,calendarMonth,selectedDay,"remaining")}`} active={selectedKpi==="remaining"}/></div>

    {selectedKpi?<section className="mccs-card mt-5 overflow-hidden rounded-2xl"><div className="flex items-center justify-between border-b px-6 py-4 dark:border-slate-800"><div><div className="text-[11px] font-extrabold uppercase tracking-[.15em] text-blue-700">KPI Drill-Down</div><h2 className="mt-1 text-lg font-bold">{selectedKpi==="committed"?"Committed Purchase Orders":selectedKpi==="certified"?"Certified Invoices":selectedKpi==="paid"?"Paid Transactions":"Remaining Commitment by PO"}</h2><p className="mt-1 text-xs text-slate-500">Live records behind the selected dashboard KPI{selectedVendorName?` for ${selectedVendorName}`:""}.</p></div><Link href={`/dashboard?${dashboardQuery(selectedVendor,calendarMonth,selectedDay)}`} className="rounded-lg border px-3 py-2 text-xs font-bold">Close list</Link></div><div className="overflow-x-auto">{selectedKpi==="committed"?<table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900"><tr><th className="px-5 py-3">PO</th><th className="px-5 py-3">Supplier / Contractor</th><th className="px-5 py-3">Value</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Open</th></tr></thead><tbody className="divide-y dark:divide-slate-800">{pos.map((x:any)=><tr key={x.id}><td className="px-5 py-4 font-bold">{x.po_number}</td><td className="px-5 py-4">{vmap.get(String(x.vendor_id))||"—"}</td><td className="px-5 py-4 font-semibold">{cMap.get(String(x.currency_id))||""} {Number(x.current_value||0).toLocaleString()}</td><td className="px-5 py-4">{x.status||"—"}</td><td className="px-5 py-4"><Link href={`/purchase-orders/${x.id}`} className="font-bold text-blue-700 hover:underline">Open</Link></td></tr>)}</tbody></table>:selectedKpi==="certified"?<table className="w-full min-w-[950px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900"><tr><th className="px-5 py-3">Invoice</th><th className="px-5 py-3">Supplier / Contractor</th><th className="px-5 py-3">PO</th><th className="px-5 py-3">Certified</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Open</th></tr></thead><tbody className="divide-y dark:divide-slate-800">{invoices.map((x:any)=>{const po=pmap.get(String(x.purchase_order_id));return <tr key={x.id}><td className="px-5 py-4 font-bold">{x.invoice_number}</td><td className="px-5 py-4">{vmap.get(String(po?.vendor_id))||"—"}</td><td className="px-5 py-4">{po?.po_number||"—"}</td><td className="px-5 py-4 font-bold text-blue-700">{cMap.get(String(x.currency_id))||""} {Number(x.certified_amount||0).toLocaleString()}</td><td className="px-5 py-4">{String(x.status||"—").replaceAll("_"," ")}</td><td className="px-5 py-4"><Link href={`/invoices/${x.id}`} className="font-bold text-blue-700 hover:underline">Open</Link></td></tr>})}</tbody></table>:selectedKpi==="paid"?<table className="w-full min-w-[950px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900"><tr><th className="px-5 py-3">Date</th><th className="px-5 py-3">Supplier / Contractor</th><th className="px-5 py-3">PO</th><th className="px-5 py-3">Amount</th><th className="px-5 py-3">Reference</th></tr></thead><tbody className="divide-y dark:divide-slate-800">{payments.map((x:any)=>{const po=pmap.get(String(x.purchase_order_id));return <tr key={x.id}><td className="px-5 py-4 font-semibold">{formatDate(x.payment_date)}</td><td className="px-5 py-4">{vmap.get(String(po?.vendor_id))||"—"}</td><td className="px-5 py-4">{po?.po_number||"—"}</td><td className="px-5 py-4 font-bold text-emerald-700">{cMap.get(String(x.currency_id))||""} {Number(x.paid_amount||0).toLocaleString()}</td><td className="px-5 py-4">{x.payment_reference||"—"}</td></tr>})}</tbody></table>:<table className="w-full min-w-[950px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900"><tr><th className="px-5 py-3">PO</th><th className="px-5 py-3">Supplier / Contractor</th><th className="px-5 py-3">Committed</th><th className="px-5 py-3">Paid</th><th className="px-5 py-3">Remaining</th></tr></thead><tbody className="divide-y dark:divide-slate-800">{pos.map((x:any)=>{const committedValue=Number(x.current_value||0),paidValue=payments.filter((p:any)=>String(p.purchase_order_id)===String(x.id)).reduce((a:number,p:any)=>a+Number(p.paid_amount||0),0),rem=Math.max(committedValue-paidValue,0),cur=cMap.get(String(x.currency_id))||"";return <tr key={x.id}><td className="px-5 py-4 font-bold">{x.po_number}</td><td className="px-5 py-4">{vmap.get(String(x.vendor_id))||"—"}</td><td className="px-5 py-4">{cur} {committedValue.toLocaleString()}</td><td className="px-5 py-4 text-emerald-700">{cur} {paidValue.toLocaleString()}</td><td className="px-5 py-4 font-bold text-blue-700">{cur} {rem.toLocaleString()}</td></tr>})}</tbody></table>}</div></section>:null}
    ...
    <section className="mccs-card my-5 overflow-hidden rounded-2xl">
      ...
      <div className="grid xl:grid-cols-[220px_minmax(0,1fr)_330px]">
        <div className="border-r p-5 dark:border-slate-800"><div className="text-[11px] font-extrabold uppercase tracking-[.15em] text-slate-500">Milestone Summary</div><div className="mt-4 space-y-2"><SummaryPill label="Overdue" value={overdueCount} tone="red"/><SummaryPill label="Due This Week" value={dueThisWeekCount} tone="amber"/><SummaryPill label="Due This Month" value={dueThisMonthCount} tone="blue"/><SummaryPill label="Next 3 Months" value={nextThreeMonthsCount} tone="emerald"/><SummaryPill label="All Outstanding" value={outstandingSorted.length} tone="slate"/></div></div>
...
function dashboardQuery(vendor:string,month:string,day:string){const p=new URLSearchParams();if(vendor)p.set("vendor",vendor);if(month)p.set("month",month);if(day)p.set("day",day);return p.toString();}
function kpiQuery(vendor:string,month:string,day:string,kpi:string){const p=new URLSearchParams(dashboardQuery(vendor,month,day));if(kpi)p.set("kpi",kpi);return p.toString();}
function SummaryPill({label,value,tone}:{label:string;value:number;tone:"red"|"amber"|"blue"|"emerald"|"slate"}){const styles={red:"bg-red-50 text-red-700",amber:"bg-amber-50 text-amber-700",blue:"bg-blue-50 text-blue-700",emerald:"bg-emerald-50 text-emerald-700",slate:"bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"};return <div className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-bold ${styles[tone]}`}><span>{label}</span><span>{value}</span></div>}
...
function MoneyCard({title,rows,helper,icon:Icon,href,active=false}:{title:string;rows:MoneyRow[];helper:string;icon:any;href:string;active?:boolean}){return <Link href={href} className={`mccs-card group block rounded-2xl p-5 transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-lg ${active?"border-blue-400 ring-2 ring-blue-100 dark:ring-blue-900/30":""}`}><div className="flex justify-between"><div className="text-[11px] font-extrabold uppercase tracking-[.15em] text-slate-500">{title}</div><div className="rounded-xl bg-blue-50 p-2.5 text-blue-700"><Icon className="h-5 w-5"/></div></div><div className="mt-3 space-y-1">{rows.slice(0,3).map(([c,v])=><div key={c} className="flex items-baseline justify-between"><span className="text-sm font-bold text-blue-700">{c}</span><span className="text-xl font-bold">{v.toLocaleString()}</span></div>)}{!rows.length?<div className="text-2xl font-bold">0</div>:null}</div><div className="mt-4 flex items-center justify-between text-xs text-slate-500"><span>{helper}</span><span className="font-bold text-blue-700 opacity-0 transition group-hover:opacity-100">View list →</span></div></Link>}
