...
export default async function PurchaseOrdersPage({ searchParams }: { searchParams: Promise<{ warning?: string }> }) {
  ...
  const poResult = await supabase
    ...
    .select("id,po_number,po_date,current_value,status,is_historical,is_deleted,vendors(vendor_name),currencies(code)")
            ...
            <tbody className="divide-y divide-slate-100">
              {pos.map((po: any) => (
                <tr key={po.id} className="bg-white">
                  <td className="px-5 py-4 font-bold text-slate-900"><Link href={`/purchase-orders/${po.id}`} className="text-blue-700 hover:underline">{po.po_number}</Link></td>
