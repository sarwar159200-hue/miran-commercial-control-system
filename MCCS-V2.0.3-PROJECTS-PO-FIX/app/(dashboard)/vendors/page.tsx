...
import { Building2, Network, Plus, GitBranch } from "lucide-react";
...
export default async function VendorsPage() {
  ...
  const names = new Map(vendors.map((v: any) => [v.id, v.vendor_name]));
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
              <tr><th className="px-5 py-3">Code</th><th className="px-5 py-3">Vendor</th><th className="px-5 py-3">Parent</th><th className="px-5 py-3">Relationship</th><th className="px-5 py-3">Status</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {vendors.map((v:any)=>(
                <tr key={v.id} className="bg-white">
                  ...
                  <td className="px-5 py-4"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">{v.is_active ? "Active" : "Inactive"}</span></td>
                </tr>
              ))}
              {!vendors.length ? <tr><td colSpan={5} className="px-5 py-12 text-center text-slate-500">No vendors yet.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </div>
