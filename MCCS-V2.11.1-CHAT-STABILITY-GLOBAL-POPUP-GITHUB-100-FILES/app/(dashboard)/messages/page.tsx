    ...
    void (async () => {
      try {
        ...
        const { data: docRows, error: docError } = await supabase
          .from("documents")
          .select("id,document_title,file_name,document_type")
          .eq("is_deleted", false)
        ...
        if (!active) return;
        if (docError) {
          console.warn("MCCS chat document list:", docError.message);
          setDocuments([]);
        } else {
          setDocuments(Array.isArray(docRows) ? (docRows as ChatDocument[]) : []);
        }
      } catch (error: any) {
                      ...
                      >
                        ...
                        {document ? (
                          ...
                          >
                            ...
                            {document.document_title || document.file_name || "MCCS document"}
                          </a>
                        ) : null}
