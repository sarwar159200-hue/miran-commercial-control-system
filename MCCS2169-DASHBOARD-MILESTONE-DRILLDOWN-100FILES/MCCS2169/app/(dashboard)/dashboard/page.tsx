...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string;month?:string;day?:string;summary?:string}>}){
  ...
  const summaryRows=summaryKey==="overdue"?outstandingSorted.filter((m:any)=>String(m.payment_due_date)<todayKey):summaryKey==="week"?outstandingSorted.filter((m:any)=>String(m.payment_due_date)>=todayKey&&String(m.payment_due_date)<=weekLimitKey):summaryKey==="month"?outstandingSorted.filter((m:any)=>String(m.payment_due_date).startsWith(defaultMonth)):summaryKey==="three-months"?outstandingSorted.filter((m:any)=>String(m.payment_due_date)>=todayKey&&String(m.payment_due_date)<=threeMonthLimitKey):summaryKey==="all"?outstandingSorted:[];
