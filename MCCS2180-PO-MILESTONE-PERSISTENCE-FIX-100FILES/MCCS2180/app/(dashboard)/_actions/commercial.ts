...
export async function updateDashboardMilestone(formData: FormData) {
  const a = await currentAccess();
  const roleCodes = new Set([...a.roleCodes].map((x:any)=>String(x).toLowerCase()));
  const canEdit = a.isSuperAdmin || ["accounts_payable","account_payable","accounts-payable","ap"].some(r=>roleCodes.has(r));
  if (!canEdit) throw new Error("Only Super Admin or Accounts Payable can update milestone payment status.");

  const milestoneId = text(formData, "milestone_id");
  if (!milestoneId) throw new Error("Milestone ID is required.");
  const allowedStatuses = new Set(["planned","awaiting_evidence","eligible","awaiting_invoice","under_verification","certified","submitted_to_ap","paid","on_hold","cancelled"]);
  const status = text(formData, "status") || "planned";
  if (!allowedStatuses.has(status)) throw new Error("Invalid milestone status.");
  const actualPaymentDate = nullable(formData, "actual_payment_date");

  const { error } = await a.supabase.from("payment_milestones").update({
    status,
...
export async function purgeDeletedPurchaseOrder(formData: FormData) {
  ...
  const poId = text(formData, "po_id");
  const reason = text(formData, "delete_reason") || "Permanent purge by Super Admin";
  if (!poId) redirect("/purchase-orders?view=history&error=PO%20ID%20is%20required");

  // Super Admin has already been authenticated above. Use the service-role client
  // for the actual purge so RLS cannot leave a partially-deleted history record.
  const db = adminClient();
  const { data: row, error: loadError } = await db.from("purchase_orders").select("id,po_number,is_deleted").eq("id", poId).maybeSingle();
  if (loadError) redirect(`/purchase-orders?view=history&error=${encodeURIComponent(loadError.message)}`);
  if (!row) redirect("/purchase-orders?view=history&error=Purchase%20Order%20not%20found");
  if (!row.is_deleted) redirect("/purchase-orders?error=Only%20soft-deleted%20POs%20can%20be%20permanently%20purged");

  const now = new Date().toISOString();
  const { data: invoiceRows } = await db.from("invoices").select("id").eq("purchase_order_id", poId);
  const invoiceIds = (invoiceRows || []).map((x:any)=>String(x.id));

  // Delete children first to satisfy foreign keys. This is intentionally limited
  // to the selected deleted PO and is available only to Super Admin.
  if (invoiceIds.length) {
    const approvalDelete = await db.from("invoice_approvals").delete().in("invoice_id", invoiceIds);
    if (approvalDelete.error) redirect(`/purchase-orders?view=history&error=${encodeURIComponent("Invoice approval purge failed: " + approvalDelete.error.message)}`);
  }

  const deletes = [
    ["documents", db.from("documents").delete().eq("purchase_order_id", poId)],
    ["payments", db.from("payments").delete().eq("purchase_order_id", poId)],
    ["invoices", db.from("invoices").delete().eq("purchase_order_id", poId)],
    ["payment milestones", db.from("payment_milestones").delete().eq("purchase_order_id", poId)],
    ["PO items", db.from("purchase_order_items").delete().eq("purchase_order_id", poId)],
    ["PO revisions", db.from("po_revisions").delete().eq("purchase_order_id", poId)],
  ] as const;

  for (const [label, request] of deletes) {
    const result:any = await request;
    if (result.error) redirect(`/purchase-orders?view=history&error=${encodeURIComponent(`${label} purge failed: ${result.error.message}`)}`);
  }

  const before = { po_number: row.po_number, purge_reason: reason, purged_at: now };
  const { error } = await db.from("purchase_orders").delete().eq("id", poId).eq("is_deleted", true);
  if (error) redirect(`/purchase-orders?view=history&error=${encodeURIComponent(error.message)}`);

  // Retain one minimal audit record after the commercial record itself is removed.
  try {
    await db.from("audit_logs").insert({
      actor_user_id:user.id,
      entity_type:"purchase_order_purge",
      entity_id:poId,
      action:"PERMANENT_PURGE_PURCHASE_ORDER",
      before_data:before,
      after_data:{permanently_deleted:true}
    });
  } catch {}
...
export async function updateInvoice(formData: FormData) {
  ...
  // Keep the controlled workflow compatible with existing approval logic while
  // ensuring the workflow immediately reflects a Super Admin status edit.
  const workflowByStatus:Record<string,string>={
    received:"received",
    under_verification:"under_verification",
    approved:"approved_for_payment",
    submitted_to_ap:"submitted_to_ap",
    paid:"paid",
    on_hold:"returned",
    rejected:"returned"
  };
  const workflowStatus=workflowByStatus[status]||status;
...
export async function updatePurchaseOrderDetails(formData: FormData){
 ...
 for(let i=0;i<milestoneCount;i++){
   const name=text(formData,`milestone_name_${i}`).trim();
   if(!name) continue;

   const milestoneId=nullable(formData,`milestone_id_${i}`);
   const pctRaw=text(formData,`milestone_percentage_${i}`).trim();
   const amountRaw=text(formData,`milestone_amount_${i}`).trim();
   const pct=pctRaw===""?null:Number(pctRaw.replace(/,/g,""));
   const amount=amountRaw===""?null:Number(amountRaw.replace(/,/g,""));

   if(pct!=null && (!Number.isFinite(pct) || pct<0 || pct>100)){
     redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`Invalid milestone percentage for "${name}".`)}`);
   }
   if(amount!=null && !Number.isFinite(amount)){
     redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`Invalid fixed amount for "${name}".`)}`);
   }
   ...
   submitted.push({
     ...
     planned_due_date:nullable(formData,`milestone_due_${i}`),
     payment_due_date:nullable(formData,`milestone_payment_due_${i}`),
     status:text(formData,`milestone_status_${i}`)||"planned",
     sequence_no:i+1
   });
 }

 if(totalPct>100.0001){
   redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`Milestone percentages total ${totalPct.toFixed(2)}%. Total allocation cannot exceed 100%.`)}`);
 }

 // Persist existing and newly-added milestones using the service-role client after
 // Super Admin permission validation. The previous version could report success
 // even when new rows were not actually persisted.
 const existing=submitted.filter((m:any)=>m.id);
 const fresh=submitted.filter((m:any)=>!m.id);

 if(existing.length){
   const results=await Promise.all(existing.map((m:any)=>
     milestoneDb.from("payment_milestones").update({
       ...
       updated_at:now,
       updated_by:user.id
     }).eq("id",m.id).eq("purchase_order_id",id)
   ));
   const failed=results.find((r:any)=>r.error);
   if(failed?.error){
     redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`Milestone update failed: ${failed.error.message}`)}`);
   }
 }

 if(fresh.length){
   const payload=fresh.map((m:any)=>({
   ...
   const { data:inserted, error:insertError }=await milestoneDb
     .from("payment_milestones")
     .insert(payload)
     .select("id,purchase_order_id,milestone_name");

   if(insertError){
     redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`New milestone save failed: ${insertError.message}`)}`);
   }
   if(!inserted || inserted.length!==payload.length){
     redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent("New milestone save was incomplete. Please try again.")}`);
   }
 }

 // Final verification: reload the PO's active milestone rows before showing success.
 const { data:verifyRows, error:verifyError }=await milestoneDb
   .from("payment_milestones")
   .select("id,milestone_name,is_deleted")
   .eq("purchase_order_id",id)
   .eq("is_deleted",false);

 if(verifyError){
   redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`Milestone verification failed: ${verifyError.message}`)}`);
 }

 const savedNames=new Set((verifyRows||[]).map((r:any)=>String(r.milestone_name||"").trim()));
 const missing=submitted.filter((m:any)=>!savedNames.has(m.milestone_name));
 if(missing.length){
   redirect(`/purchase-orders/${id}/edit?error=${encodeURIComponent(`Milestone persistence check failed for: ${missing.map((m:any)=>m.milestone_name).join(", ")}`)}`);
 }
 ...
 revalidatePath("/dashboard");
 revalidatePath("/reports");
 redirect(`/purchase-orders/${id}/edit?saved=1`);
}
