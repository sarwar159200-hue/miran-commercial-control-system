...
export async function POST(request: Request) {
  try {
    ...
    const uploadToken=secret?`${expiresAt}.${createHmac("sha256",secret).update(`${session.sessionUrl}|${expiresAt}`).digest("hex")}`:"";
    return NextResponse.json({ ok: true, ...session, uploadToken, uploadName, title: baseTitle, invoicePurchaseOrderId: invoice?.purchase_order_id || null });
  } catch (error) {
