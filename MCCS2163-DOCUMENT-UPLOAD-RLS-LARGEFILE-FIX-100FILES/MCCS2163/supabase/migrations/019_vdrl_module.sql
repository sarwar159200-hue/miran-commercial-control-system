...
-- -----------------------------------------------------------------------------
-- MCCS V2.12.1 additive extension (also provided separately as 020 upgrade SQL)
-- Grouped IFR/IFA/IFC Excel mirror, status/comments sync and per-tab access.
-- -----------------------------------------------------------------------------

begin;

alter table public.vdrl_documents
  add column if not exists ifc_planned_submit_date date,
  add column if not exists ifc_actual_submit_date date,
...
create table if not exists public.user_tab_permissions (
  ...
  constraint user_tab_permissions_tab_key_check check (tab_key in ('dashboard','vendors','projects','purchase_orders','payment_milestones','invoices','payments','vdrl','documents','reports','messages','administration'))
);

insert into public.user_tab_permissions(user_id,tab_key,allowed)
select p.id, t.tab_key,
  case when t.tab_key='administration' then (
    coalesce(p.is_super_admin,false) or exists (
      select 1 from public.user_roles ur join public.roles r on r.id=ur.role_id
      where ur.user_id=p.id and lower(r.role_code) in ('admin','super_admin')
    )
  ) else true end
from public.profiles p
cross join (values ('dashboard'),('vendors'),('projects'),('purchase_orders'),('payment_milestones'),('invoices'),('payments'),('vdrl'),('documents'),('reports'),('messages'),('administration')) as t(tab_key)
...
drop policy if exists user_tab_permissions_read_self_or_admin on public.user_tab_permissions;
create policy user_tab_permissions_read_self_or_admin on public.user_tab_permissions for select to authenticated using (user_id=auth.uid() or public.vdrl_is_super_admin());
drop policy if exists user_tab_permissions_write_super_admin on public.user_tab_permissions;
create policy user_tab_permissions_write_super_admin on public.user_tab_permissions for all to authenticated using (public.vdrl_is_super_admin()) with check (public.vdrl_is_super_admin());

create or replace function public.vdrl_enforce_status_override()
returns trigger language plpgsql set search_path=public as $$
begin
  if new.status_override is not null and btrim(new.status_override) <> '' then
    new.current_status := new.status_override;
    if new.status_override in ('approved','cancelled','on_hold') then new.responsible_party := 'Closed'; new.action_due_date := null; new.days_overdue := 0;
    elsif new.status_override in ('under_miran_review','overdue_return') then new.responsible_party := 'Miran';
    elsif new.status_override in ('pending_submission','returned_for_revision','overdue_submission','overdue_resubmission') then new.responsible_party := 'Vendor';
    end if;
  end if;
  return new;
end; $$;

drop trigger if exists trg_vdrl_enforce_status_override on public.vdrl_documents;
create trigger trg_vdrl_enforce_status_override before insert or update on public.vdrl_documents for each row execute function public.vdrl_enforce_status_override();

create or replace function public.vdrl_apply_extended_batch(p_batch uuid)
returns integer language plpgsql security definer set search_path=public as $$
declare changed integer := 0;
begin
  if not public.vdrl_is_super_admin() then raise exception 'Super Admin permission is required.'; end if;
  update public.vdrl_documents d set
    ifc_planned_submit_date=nullif(r.mapped_data->>'ifc_planned_submit_date','')::date,
    ifc_actual_submit_date=nullif(r.mapped_data->>'ifc_actual_submit_date','')::date,
    status_override=nullif(r.mapped_data->>'status_override',''),
    comments=nullif(r.mapped_data->>'comments',''),
    updated_at=now(),updated_by=auth.uid()
  from public.vdrl_upload_rows r join public.vdrl_upload_batches b on b.id=r.batch_id
  where r.batch_id=p_batch and jsonb_array_length(r.validation_errors)=0 and d.register_id=b.register_id and d.business_key=upper(trim(coalesce(r.mapped_data->>'document_number','')));
  get diagnostics changed = row_count;
  return changed;
end; $$;
grant execute on function public.vdrl_apply_extended_batch(uuid) to authenticated;
