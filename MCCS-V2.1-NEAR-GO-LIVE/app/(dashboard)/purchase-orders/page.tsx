...
export default async function PurchaseOrdersPage({ searchParams }: { searchParams: Promise<{ warning?: string }> }) {
  ...
  const poResult = await supabase
    ...
    .select("id,po_number,po_date,current_value,status,is_historical,is_deleted,vendor_id,currency_id,project_id")
    .order("po_date", { ascending: false });

  const pos = (poResult.data ?? []).filter((po:any)=>!po.is_deleted);
  const vendorIds=[...new Set(pos.map((p:any)=>p.vendor_id).filter(Boolean))];
  const currencyIds=[...new Set(pos.map((p:any)=>p.currency_id).filter(Boolean))];
  ...
  const vendors=new Map((vr.data??[]).map((x:any)=>[x.id,x.vendor_name]));
  const currencies=new Map((cr.data??[]).map((x:any)=>[x.id,x.code]));
  const projects=new Map((pr.data??[]).map((x:any)=>[x.id,x.project_code]));
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    {poResult.error?<div className="mb-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">PO register error: {poResult.error.message}</div>:null}
