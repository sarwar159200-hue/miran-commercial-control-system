...
export default async function Page(){
 ...
 const pos=new Map((pr.data??[]).map((p:any)=>[p.id,p]));
 const cids=[...new Set((pr.data??[]).map((p:any)=>p.currency_id).filter(Boolean))];
 const cr=cids.length?await supabase.from("currencies").select("id,code").in("id",cids):{data:[]} as any; const currencies=new Map((cr.data??[]).map((c:any)=>[c.id,c.code]));
 const smap=new Map(sums.map((s:any)=>[s.purchase_order_id,s]));
