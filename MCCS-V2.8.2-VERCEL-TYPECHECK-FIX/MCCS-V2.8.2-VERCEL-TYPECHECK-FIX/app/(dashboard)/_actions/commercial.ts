...
export async function softDeletePurchaseOrder(formData: FormData) {
  ...
  const { error } = await supabase.from("purchase_orders").update({
    ...
    deleted_by: user.id,
    delete_reason: nullable(formData,"delete_reason") || "Deleted by Super Admin",
    status: "cancelled",
  }).eq("id", poId);
...
export async function deleteProjectPackage(formData: FormData){
 ...
 if(error) redirect(`/projects?error=${encodeURIComponent(error.message)}`); revalidatePath("/projects");revalidatePath("/purchase-orders/new");redirect("/projects?deleted=1");
}
...
export async function deleteVendorRecord(formData: FormData){
 ...
 if(error) redirect(`/vendors?error=${encodeURIComponent(error.message)}`); revalidatePath("/vendors");redirect("/vendors?deleted=1");
}
