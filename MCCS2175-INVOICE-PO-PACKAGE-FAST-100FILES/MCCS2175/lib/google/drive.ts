...
async function clearInvalidGoogleRefreshToken() {
  const admin = adminClient();
  await admin.from("google_drive_config").update({
    refresh_token: null,
    updated_at: new Date().toISOString(),
  }).eq("id", 1);
}
...
export async function getGoogleAccessToken() {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      oauthError = String(parsed.error || "");
    } catch {}
    if (oauthError === "invalid_grant") {
      // The stored refresh token was revoked/expired by Google. Remove it so MCCS
      // stops repeatedly attempting a dead token and clearly reports reconnection.
      await clearInvalidGoogleRefreshToken();
      throw new Error("Google Drive authorization expired or was revoked. A Super Admin must reconnect Google Drive from Administration → Google Drive Configuration, then retry the upload.");
    }
    throw new Error(`Google Drive authorization could not be refreshed (${response.status}). Please retry. If it continues, reconnect Google Drive from Administration.`);
  }
...
/** Permanently delete a Google Drive file owned/managed by the MCCS service account. */
export async function deleteGoogleDriveFile(fileId: string) {
  ...
  if (!response.ok && response.status !== 404) {
    const text = await response.text();
    throw new Error(`Google Drive file delete failed (${response.status}): ${text}`);
  }
}

/** Replace the contents of an existing Google Drive file while keeping the same file ID/link. */
export async function updateGoogleDriveFile(fileId: string, content: ArrayBuffer, mimeType: string) {
  const token = await getGoogleAccessToken();
  const response = await fetch(`${GOOGLE_UPLOAD_API}/files/${encodeURIComponent(fileId)}?uploadType=media&fields=id,name,size,webViewLink,webContentLink`, {
    method: "PATCH",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": mimeType || "application/octet-stream" },
    body: new Blob([content], { type: mimeType || "application/octet-stream" }),
    cache: "no-store",
  });
  const text = await response.text();
  if (!response.ok) throw new Error(`Google Drive file update failed (${response.status}): ${text}`);
  return JSON.parse(text) as { id: string; name?: string; size?: string; webViewLink?: string; webContentLink?: string };
}
...
/**
 * Securely fetch a Google Drive file through the MCCS server account.
 * Normal MCCS users never need direct Google Drive permissions.
 */
export async function getGoogleDriveFileMetadata(fileId: string) {
  const response = await driveFetch(`/files/${encodeURIComponent(fileId)}?fields=id,name,mimeType,size,trashed`);
  const text = await response.text();
  if (!response.ok) throw new Error(`Google Drive file metadata lookup failed (${response.status}): ${text}`);
  return JSON.parse(text) as { id: string; name?: string; mimeType?: string; size?: string; trashed?: boolean };
}

export async function streamGoogleDriveFile(fileId: string, range?: string | null) {
  const token = await getGoogleAccessToken();
  const headers = new Headers({ Authorization: `Bearer ${token}` });
  if (range) headers.set("Range", range);
  return fetch(`${GOOGLE_DRIVE_API}/files/${encodeURIComponent(fileId)}?alt=media`, {
    method: "GET",
    headers,
    cache: "no-store",
  });
...
/** Move an existing Drive file between controlled folders without exposing permissions to MCCS users. */
export async function moveGoogleDriveFile(fileId: string, addParentId: string, removeParentId?: string | null) {
  if (!fileId || !addParentId) return;
  ...
  const response = await driveFetch(`/files/${encodeURIComponent(fileId)}?${params.toString()}`, { method: "PATCH" });
  const text = await response.text();
  if (!response.ok) throw new Error(`Google Drive move failed (${response.status}): ${text}`);
  return JSON.parse(text) as { id: string; parents?: string[] };
}
