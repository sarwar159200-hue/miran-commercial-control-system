...
export function PresenceHeartbeat(){
  ...
  async function handleSignal(s:Signal){
    ...
    if(s.signal_type==="resume-request"){
      ...
      const p=await peerFor(s.call_id,s.sender_id);await addMic(p);if(p.signalingState!=="stable"){try{await p.setLocalDescription({type:"rollback"} as RTCSessionDescriptionInit)}catch{}}const offer=await p.createOffer({iceRestart:true});await p.setLocalDescription(offer);await sendSignal(s.call_id,s.sender_id,"resume-offer",offer);return
    }
    if(s.signal_type==="resume-offer"){
      ...
      const p=await peerFor(s.call_id,s.sender_id);await addMic(p);await p.setRemoteDescription(new RTCSessionDescription(s.payload));await flushIce(p);const a=await p.createAnswer();await p.setLocalDescription(a);await sendSignal(s.call_id,s.sender_id,"resume-answer",a);return
    }
    if(s.signal_type==="offer"){
      if(callId.current!==s.call_id)return;
      const p=await peerFor(s.call_id,s.sender_id);await addMic(p);if(p.signalingState!=="stable"){try{await p.setLocalDescription({type:"rollback"} as RTCSessionDescriptionInit)}catch{}}await p.setRemoteDescription(new RTCSessionDescription(s.payload));await flushIce(p);const a=await p.createAnswer();await p.setLocalDescription(a);await sendSignal(s.call_id,s.sender_id,"answer",a);return
    }
  ...
  async function share(){if(!pc.current||!callId.current)return;try{const s=await navigator.mediaDevices.getDisplayMedia({video:true,audio:false});screen.current=s;const t=s.getVideoTracks()[0];pc.current.addTrack(t,s);setLocalSharing(true);window.localStorage.setItem("mccs:screen-sharing-call",callId.current);await supabase.from("chat_calls").update({screen_shared:true}).eq("id",callId.current);await sendSignal(callId.current,partner.current,"screen-start",{});t.onended=()=>void stopShare();const o=await pc.current.createOffer();await pc.current.setLocalDescription(o);await sendSignal(callId.current,partner.current,"offer",o)}catch{setNotice("Screen sharing was cancelled.")}}
  ...
  return <>
    ...
    {activeCall?<>
      ...
      {callMinimized?<button onClick={()=>setCallMinimized(false)} className="fixed bottom-6 left-1/2 z-[145] flex -translate-x-1/2 items-center gap-3 rounded-2xl bg-slate-950 px-4 py-3 text-left text-white shadow-2xl"><div className="h-9 w-9 rounded-full bg-blue-600 text-center text-xs font-black leading-9">{initials(profiles.find(p=>p.id===partner.current))}</div><div><div className="text-[10px] font-black uppercase tracking-[.14em] text-blue-300">Voice Call</div><div className="text-sm font-black">{pname(profiles.find(p=>p.id===partner.current))}</div><div className="text-[10px] text-slate-300">{callStatus} · {dur(seconds)}{localSharing?" · Sharing screen":remoteSharing?" · Screen shared":""}</div></div><Maximize2 className="h-4 w-4 text-slate-300"/></button>:
