...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string;month?:string;day?:string}>}){
  ...
  const activePackageCount=selectedPackages.filter((x:any)=>x.is_active!==false).length;
  const inactivePackageCount=Math.max(selectedPackages.length-activePackageCount,0);
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    <section className="mb-5 rounded-2xl border bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div><div className="text-[11px] font-extrabold uppercase tracking-[.16em] text-slate-500">Supplier Package Overview</div><div className="mt-1 text-sm text-slate-500">Click a supplier name to see how many packages they have and the status of each package.</div></div>
        <div className="flex max-w-full gap-2 overflow-x-auto pb-1">
          {vendors.map(v=>{const count=(packagesByVendor.get(v.id)||[]).length;return <Link key={v.id} href={`/dashboard?${dashboardQuery(v.id,calendarMonth,"")}`} scroll={false} className={`whitespace-nowrap rounded-full border px-3 py-2 text-xs font-bold transition ${selectedVendor===v.id?"border-blue-600 bg-blue-600 text-white":"border-slate-200 bg-white text-slate-700 hover:border-blue-300 hover:text-blue-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"}`}>{v.name}<span className={`ml-2 rounded-full px-2 py-0.5 ${selectedVendor===v.id?"bg-white/20 text-white":"bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"}`}>{count}</span></Link>})}
        </div>
      </div>
      {selectedVendor?<div className="mt-4 border-t pt-4 dark:border-slate-800">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><div className="text-lg font-black">{selectedVendorName}</div><div className="mt-1 text-sm text-slate-500">{selectedPackages.length} package{selectedPackages.length===1?"":"s"} • {activePackageCount} active • {inactivePackageCount} inactive</div></div><Link href={`/projects?vendor=${encodeURIComponent(selectedVendor)}`} className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-bold text-blue-700 hover:bg-blue-100">Open supplier packages →</Link></div>
        <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{selectedPackages.map((pkg:any)=><div key={pkg.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 px-3 py-2.5 dark:border-slate-800"><div className="min-w-0"><div className="truncate text-sm font-bold">{pkg.project_name||pkg.project_code||"Unnamed Package"}</div><div className="truncate text-[11px] text-slate-500">{pkg.project_code||"No code"}</div></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${pkg.is_active!==false?"bg-emerald-50 text-emerald-700":"bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"}`}>{pkg.is_active!==false?"Active":"Inactive"}</span></div>)}{!selectedPackages.length?<div className="rounded-xl border border-dashed p-4 text-sm text-slate-500">No package is currently assigned to this supplier.</div>:null}</div>
      </div>:null}
    </section>
