...
export default async function Page({
  ...
  searchParams: Promise<{ deleted?: string; error?: string; updated?: string; bulk_updated?: string; vendor?: string; status?: string }>;
}) {
  ...
  const poIds = [...new Set(allRows.map((r: any) => r.purchase_order_id).filter(Boolean))];
  const pr = poIds.length
    ? await supabase
        .from("purchase_orders")
        .select("id,po_number,current_value,currency_id,vendor_id")
        .in("id", poIds)
        .eq("is_deleted", false)
    : ({ data: [] } as any);
  ...
  const selectedStatus = q.status || "";
  const rows = allRows.filter((m:any)=>{
    const vendorOk=!selectedVendorId||String(pos.get(String(m.purchase_order_id))?.vendor_id||"")===selectedVendorId;
    const statusOk=!selectedStatus||String(m.status||"")===selectedStatus;
    return vendorOk&&statusOk;
  });

  const cids = [...new Set((pr.data ?? []).map((p: any) => p.currency_id).filter(Boolean))];
  const cr = cids.length ? await supabase.from("currencies").select("id,code").in("id", cids) : ({ data: [] } as any);
  ...
  const summaryMap = new Map<string, { po_number: string; allocated: number; remaining: number; remainingAmount: number }>();
  for (const m of rows) {
    const p = pos.get(String((m as any).purchase_order_id));
    if (!p) continue;
    const k = String(p.id);
    const cur = summaryMap.get(k) || { po_number: p.po_number, allocated: 0, remaining: 100, remainingAmount: Number(p.current_value || 0) };
  ...
  return (
    <div className="mx-auto max-w-[1700px]">
      ...
      {(selectedVendor||selectedStatus) ? (
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-blue-100 bg-blue-50/70 px-5 py-4 text-sm text-blue-900 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-100">
          <div>Showing {selectedStatus?<><strong>{selectedStatus.replaceAll("_"," ")}</strong> </>:null}payment milestones{selectedVendor?<> for <strong>{selectedVendor.vendor_name}</strong></>:null}. {rows.length} active milestone{rows.length===1?"":"s"} found.</div>
          <Link href={selectedVendor?`/payment-milestones?vendor=${encodeURIComponent(selectedVendorId)}`:"/payment-milestones"} className="rounded-lg border border-blue-200 bg-white px-3 py-2 text-xs font-bold text-blue-700">Clear status filter</Link>
        </div>
      ) : null}
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1450px] text-left text-sm">
              ...
              <tbody className="divide-y dark:divide-slate-800">
                ...
                {!rows.length ? <tr><td colSpan={9} className="px-5 py-10 text-center text-slate-500">No active milestones for this supplier.</td></tr> : null}
              </tbody>
            </table>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1200px] text-left text-sm">
            ...
            <tbody className="divide-y dark:divide-slate-800">
              ...
              {!rows.length ? <tr><td colSpan={9} className="px-5 py-12 text-center text-slate-500">No payment milestones yet.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </div>
