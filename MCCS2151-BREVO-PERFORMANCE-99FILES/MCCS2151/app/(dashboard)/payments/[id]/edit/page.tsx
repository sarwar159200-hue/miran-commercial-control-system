...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const [rr, pr, ir, cr, vr, mr, pyr, pjr] = await Promise.all([
  ...
  const pos = (pr.data ?? []).map((p: any) => ({ id: String(p.id), po_number: String(p.po_number), vendor_id: p.vendor_id ? String(p.vendor_id) : null, vendor_name: vendorMap.get(String(p.vendor_id)) || "Unknown Supplier", currency_id: p.currency_id ? String(p.currency_id) : null, current_value: Number(p.current_value || 0), project_id: p.project_id ? String(p.project_id) : null, package_name: p.project_id ? (projectMap.get(String(p.project_id)) || "Unassigned Package") : "No linked package" }));
