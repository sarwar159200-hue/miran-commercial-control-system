...
export default async function Page() {
  ...
  const emailConfigured = Boolean(process.env.RESEND_API_KEY && process.env.MCCS_EMAIL_FROM);
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      {!emailConfigured ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Welcome-email sending is currently disabled. Add <b>RESEND_API_KEY</b> and <b>MCCS_EMAIL_FROM</b> in Vercel, then redeploy. Account creation itself remains available.</div> : <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">MCCS welcome-email delivery is configured.</div>}
