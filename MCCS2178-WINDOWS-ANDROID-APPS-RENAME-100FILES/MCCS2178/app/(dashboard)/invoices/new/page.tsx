...
export default async function Page({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [pr, vr, cr, mr, pf, ur, rr, pjr] = await Promise.all([
    ...
    s.from("profiles").select("id,full_name,preferred_name").eq("is_active", true),
    s.from("user_roles").select("user_id,role_id"),
    s.from("roles").select("id,role_code"),
    s.from("projects").select("id,project_name,project_code").eq("is_deleted", false).order("project_name"),
  ]);
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <InvoiceCreateForm pos={pos} vendors={(vr.data ?? []).map((v: any) => ({ id: String(v.id), vendor_name: String(v.vendor_name) }))} currencies={(cr.data ?? []).map((c: any) => ({ id: String(c.id), code: String(c.code) }))} milestones={milestones} reviewers={reviewers} projects={(pjr.data ?? []).map((p: any) => ({ id: String(p.id), label: String(p.project_name || p.project_code || "Unnamed Package") }))} />
  </div>;
}
