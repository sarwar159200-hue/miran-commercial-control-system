...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string;month?:string;day?:string}>}){
  ...
  const [por,mr,ir,pyr,vr,cr,pr]=await Promise.all([
    s.from("purchase_orders").select("id,po_number,vendor_id,project_id,current_value,currency_id,is_deleted"),
    ...
    s.from("projects").select("id,project_name,project_code")
  ]);
  ...
  const projectMap=new Map<string,string>((pr.data??[]).map((x:any)=>[String(x.id),String(x.project_name||x.project_code||"Unnamed Package")]));
