...
import { createSubVendor } from "../../../_actions/commercial";
...
export default async function Page({ searchParams }: { searchParams: Promise<{error?:string}> }) {
  ...
  const parentResult = await supabase.from("vendors").select("id,vendor_name").is("parent_vendor_id", null).eq("is_active", true).order("vendor_name");
  const parents = parentResult.data ?? [];

  return (
    <div className="mx-auto max-w-5xl">
      <div className="flex items-end justify-between">
        <div><div className="text-xs font-extrabold uppercase tracking-[0.2em] text-blue-700">Vendor Master</div><h1 className="mt-2 text-3xl font-bold text-slate-950">Add Sub-Vendor</h1></div>
      ...
      <form action={createSubVendor} className="mccs-card mt-7 rounded-2xl p-6">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <Field label="Parent Vendor *">
            <select name="parent_vendor_id" required className="input">
              <option value="">Select main vendor</option>
              {parents.map((v:any)=><option key={v.id} value={v.id}>{v.vendor_name}</option>)}
            </select>
          </Field>
