...
export default function SuperAdminDelete({ entity, entityId, entityLabel, action, idField, className }: Props) {
    ...
    {open ? <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl dark:bg-slate-950">
        <div className="flex items-start justify-between border-b p-5 dark:border-slate-800">
          <div className="flex gap-3"><div className="rounded-xl bg-red-50 p-2 text-red-700"><AlertTriangle className="h-5 w-5" /></div><div><h2 className="text-xl font-bold">Delete {entityLabel}?</h2><p className="mt-1 text-sm text-slate-500">Super Admin controlled deletion. Review the consequences before confirming.</p></div></div>
        ...
        <div className="space-y-5 p-5">
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:bg-amber-950/20 dark:text-amber-100">
            This is an <b>audit-safe soft delete</b>. The record will disappear from normal MCCS registers and reports, but the database audit trail will be retained. Google Drive files are not physically deleted by this action.
          </div>
