...
import { ensureVendorDriveFolder, ensureProjectDriveStructure, ensurePOFolder, ensureInvoiceFolder, uploadFileToGoogleDrive, sanitizeDriveName } from "@/lib/google/drive";
...
export async function createProject(formData: FormData) {
  ...
  if (!vendorId || !projectCode || !projectName) {
    redirect("/admin/projects/new?error=Vendor%2C%20project%20code%20and%20project%20name%20are%20required");
  }

  const { data: vendor } = await supabase.from("vendors").select("vendor_name").eq("id", vendorId).maybeSingle();
  if (!vendor?.vendor_name) redirect("/admin/projects/new?error=Selected%20vendor%20could%20not%20be%20found");
  ...
  if (error || !project) {
    redirect(`/admin/projects/new?error=${encodeURIComponent(error?.message || "Unable to create project")}`);
  }
  ...
  redirect(`/admin/projects?created=1${warning ? `&warning=${encodeURIComponent(warning)}` : ""}`);
}
