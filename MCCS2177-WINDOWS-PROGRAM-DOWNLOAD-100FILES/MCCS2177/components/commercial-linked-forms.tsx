...
export function InvoiceCreateForm({
  ...
  async function submitInvoice(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); setSaving(true); setUploadError("");
    try {
      const fd=new FormData(e.currentTarget);
      const primary=fd.get("invoice_file") as File|null;
      const supports=fd.getAll("invoice_supporting_files").filter((x):x is File=>x instanceof File&&x.size>0);
      fd.delete("invoice_file"); fd.delete("invoice_supporting_files");
      const result:any=await createInvoice(fd);
      if(!result?.id) return;
      const base={project_id:result.projectId,vendor_id:result.vendorId,purchase_order_id:result.purchaseOrderId,invoice_id:result.invoiceId,milestone_id:result.milestoneId,is_historical:result.historical,document_category:"invoice"};
      if(primary instanceof File&&primary.size>0) await uploadCommercialFile(primary,{...base,document_title:`Invoice ${String(fd.get("invoice_number")||"")}`,document_type:"Invoice PDF / Primary Invoice"});
      for(const f of supports) await uploadCommercialFile(f,{...base,document_title:f.name.replace(/\.[^.]+$/,""),document_type:"Invoice Supporting Document"});
      window.location.href=`/invoices/${result.id}?created=1`;
    } catch(err:any){setUploadError(String(err?.message||"Invoice upload failed.").includes("Google Drive authorization") ? "Google Drive authorization has expired. Please ask a Super Admin to reconnect Google Drive in Administration → Google Drive Configuration, then retry." : (err?.message||"Invoice upload failed.")); setSaving(false);}
  }

  return (
    <form onSubmit={submitInvoice} className="mccs-card mt-7 rounded-2xl p-6">
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="PO *" hint="PO number, supplier/contractor and package are shown together.">
          <select name="purchase_order_id" required className="input" value={poId} onChange={(e) => onPOChange(e.target.value)}>
            <option value="">Select PO</option>
            {pos.map((p) => (
              <option key={p.id} value={p.id}>{p.po_number} — {p.vendor_name} — {p.package_name || "Package not set"}</option>
            ))}
          </select>
        </Field>
        ...
        <Field label="Project / Package *" hint="Automatically selected from the PO. Super Admin can change it for this invoice without changing the PO.">
          <select name="project_id" required className="input" value={projectId} onChange={(e) => setProjectId(e.target.value)} disabled={!poId}>
      ...
      <div className="mt-7 flex justify-end"><button disabled={saving} className="rounded-xl bg-[#07111f] px-5 py-3 text-sm font-bold text-white disabled:opacity-50 dark:bg-blue-600">{saving?"Creating & uploading…":"Create Invoice & Drive Folder"}</button></div>
    </form>
  );
}
    ...
    <form action={action} className="mccs-card mt-7 rounded-2xl p-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="PO *" hint="Supplier/contractor is displayed with the PO number.">
          <select name="purchase_order_id" required className="input" value={poId} onChange={(e) => onPOChange(e.target.value)}>
            <option value="">Select PO</option>
            {pos.map((p) => <option key={p.id} value={p.id}>{p.po_number} — {p.vendor_name} — {p.package_name || "Package not set"}</option>)}
          </select>
        </Field>
        ...
        <Field label="Project / Package" hint="Automatically filled from the selected PO. Super Admin can change it by selecting a different PO when editing the payment."><input className="input bg-slate-50 dark:bg-slate-900" value={selectedPO?.package_name || "Select a PO first"} readOnly /></Field>
      </div>
