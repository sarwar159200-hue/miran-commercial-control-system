...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string;month?:string;day?:string;summary?:string;fpo?:string;fsupplier?:string;fpackage?:string;fmilestone?:string;fpct?:string;fdue?:string;foverdue?:string;foutstanding?:string;fstatus?:string;factual?:string}>}){
  ...
  for(const po of pos){
    ...
    for(const m of poMilestones){
      // A milestone explicitly marked Paid is settled. Do not keep it in the
      // outstanding / overdue population even when a legacy payment record is
      // not linked to the milestone. The dashboard status editor therefore
      // immediately drives all summary KPIs and drill-downs consistently.
      if(String(m.status||"").trim().toLowerCase()==="paid") continue;
