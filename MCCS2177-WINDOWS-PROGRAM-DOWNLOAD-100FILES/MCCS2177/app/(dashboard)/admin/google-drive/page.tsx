...
export default async function Page({ searchParams }: { searchParams: Promise<{ connected?: string; error?: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <section className="mccs-card mt-6 rounded-2xl p-6"><h2 className="text-lg font-bold">Google Drive authorization</h2><p className="mt-2 text-sm text-slate-500">Only a Super Admin should connect the storage account. MCCS requests the restricted <code>drive.file</code> scope.</p>{!status.connected&&status.configured?<div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-800">Google Drive authorization is not active. Reconnect once, then document, PO and invoice uploads will resume.</div>:null}<a href="/api/auth/google/" className="mt-5 inline-flex rounded-xl bg-[#07111f] px-5 py-3 text-sm font-bold text-white dark:bg-blue-600">{status.connected ? "Reconnect Google Drive" : "Reconnect Google Drive"}</a></section>
  </div>;
}
