...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{vendor?:string}>}){
  ...
  const [por,mr,ir,pyr,vr,cr]=await Promise.all([
    ...
    s.from("payments").select("id,purchase_order_id,paid_amount,currency_id,payment_date,payment_reference,is_deleted").eq("is_deleted",false),
  ...
  // Allocate actual payments against contractual milestones oldest-first for each PO.
  // This prevents an already-paid milestone from continuing to appear as the "next payment due".
  const paidByPo=new Map<string,number>();
  payments.forEach((p:any)=>paidByPo.set(String(p.purchase_order_id),(paidByPo.get(String(p.purchase_order_id))||0)+Number(p.paid_amount||0)));
  const outstandingMilestones:any[]=[];
  for(const po of pos){
    let credit=paidByPo.get(String(po.id))||0;
    ...
    for(const m of poMilestones){
      ...
      const applied=Math.min(Math.max(credit,0),Math.max(fullAmount,0));
      const outstanding=Math.max(fullAmount-applied,0);
      credit=Math.max(credit-applied,0);
      if(outstanding>0.005)outstandingMilestones.push({...m,_full_amount:fullAmount,_remaining_amount:outstanding});
    }
  }
