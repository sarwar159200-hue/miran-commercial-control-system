...
import { ensureVendorDriveFolder, ensureProjectDriveStructure, ensurePOFolder, ensureInvoiceFolder, uploadFileToGoogleDrive, sanitizeDriveName, ensureGoogleFolderPath, moveGoogleDriveFile } from "@/lib/google/drive";
...
export async function createPurchaseOrder(formData: FormData) {
  ...
  try {
    ...
    const folderParts = [vendor.vendor_name, project.project_name, "PO", poNumber];
    const primaryPo = formData.get("po_pdf");
    if (primaryPo instanceof File && primaryPo.size > 0) await saveCommercialFile({ supabase, userId:user.id, file:primaryPo, folderParts, projectId, vendorId, purchaseOrderId:po.id, documentTitle:`Purchase Order ${poNumber}`, documentType:"Purchase Order PDF", documentCategory:"po", isHistorical:historical });
    const technicalOffer = formData.get("technical_offer");
    if (technicalOffer instanceof File && technicalOffer.size > 0) await saveCommercialFile({ supabase, userId:user.id, file:technicalOffer, folderParts, projectId, vendorId, purchaseOrderId:po.id, documentTitle:"Technical Offer", documentType:"Technical Offer", documentCategory:"po", isHistorical:historical });
    const supportFiles = formData.getAll("supporting_files").filter((f): f is File => f instanceof File && f.size > 0);
    for (const f of supportFiles) await saveCommercialFile({ supabase, userId:user.id, file:f, folderParts, projectId, vendorId, purchaseOrderId:po.id, documentTitle:f.name.replace(/\.[^.]+$/, ""), documentType:"Supporting Document", documentCategory:"po", isHistorical:historical });
  } catch (driveError) {
  ...
  redirect(`/purchase-orders/${po.id}?created=1${warning ? `&warning=${encodeURIComponent(warning)}` : ""}`);
}
...
export async function createInvoice(formData: FormData) {
  ...
  const [{ data: vendor }, { data: project }] = await Promise.all([
    ...
    supabase.from("projects").select("project_name").eq("id", po.project_id).maybeSingle(),
  ]);
  ...
  try {
    ...
    const folderParts = [vendor.vendor_name, project.project_name, "Invoice", invoiceNumber];
    const primary = formData.get("invoice_file");
    if (primary instanceof File && primary.size > 0) await saveCommercialFile({ supabase, userId:user.id, file:primary, folderParts, projectId:String(po.project_id || ""), vendorId:effectiveVendorId, purchaseOrderId, invoiceId:created.id, milestoneId:selectedMilestoneId, documentTitle:`Invoice ${invoiceNumber}`, documentType:"Invoice PDF / Primary Invoice", documentCategory:"invoice", isHistorical:historical });
    const supportFiles = formData.getAll("invoice_supporting_files").filter((f): f is File => f instanceof File && f.size > 0);
    for (const f of supportFiles) await saveCommercialFile({ supabase, userId:user.id, file:f, folderParts, projectId:String(po.project_id || ""), vendorId:effectiveVendorId, purchaseOrderId, invoiceId:created.id, milestoneId:selectedMilestoneId, documentTitle:f.name.replace(/\.[^.]+$/, ""), documentType:"Invoice Supporting Document", documentCategory:"invoice", isHistorical:historical });
  } catch (driveError) {
  ...
  redirect(`/invoices/${created.id}?created=1${warning ? `&warning=${encodeURIComponent(warning)}` : ""}`);
}
...
// -----------------------------------------------------------------------------
// MCCS V2.12.0 - Integrated VDRL module
// -----------------------------------------------------------------------------

const VDRL_FIELDS = [
  "document_number","document_title","discipline","sub_discipline","document_type","stage","planned_submit_date","actual_submit_date","planned_return_date","actual_return_date","revision","return_code","resubmission_due_date"
] as const;
...
const VDRL_ALIASES: Record<VdrlField,string[]> = {
  ...
  planned_submit_date:["planned submit date","plan submit date","planned date","forecast date","submission date","target submission","planned submission"],
  actual_submit_date:["actual submit date","actual submission date","vendor submission date","submitted date","actual issue date"],
  planned_return_date:["planned return date","plan return date","return due date","review due date","miran return date"],
  actual_return_date:["actual return date","returned date","miran actual return","actual review return"],
  revision:["revision","rev","version","current revision"],
  return_code:["return code","returned with code","review code","code","status code"],
  resubmission_due_date:["resubmission due date","resubmit due date","next submission due","re-submit due date"]
};
...
async function classifyVdrlRows(supabase:any, registerId:string, rows:Array<{row_no:number;raw_data:any;mapped_data:any;validation_errors:any[]}>) {
  const { data: existing } = await supabase.from("vdrl_documents").select("business_key,document_title,discipline,sub_discipline,document_type,current_stage,current_revision,planned_submit_date,actual_submit_date,planned_return_date,actual_return_date,resubmission_due_date,return_code").eq("register_id",registerId).eq("is_active",true).limit(20000);
...
async function fetchAllVdrlDocs(supabase:any, registerId:string) {
  ...
  for(let from=0;;from+=1000){
    const {data,error}=await supabase.from("vdrl_documents").select("document_number,document_title,discipline,sub_discipline,document_type,current_stage,current_revision,planned_submit_date,actual_submit_date,planned_return_date,actual_return_date,resubmission_due_date,return_code,current_status,submission_variance,return_variance,days_overdue,responsible_party,action_due_date").eq("register_id",registerId).eq("is_active",true).order("document_number").range(from,from+999);
...
async function syncVdrlControlledOutput(supabase:any, registerId:string, userId:string) {
  ...
  try {
    const archiveId = await ensureGoogleFolderPath(["VDRL",supplierName,packageName,"Archive"]);
    const currentId = await ensureGoogleFolderPath(["VDRL",supplierName,packageName,"Current"]);
    if (register.current_controlled_file_id) await moveGoogleDriveFile(register.current_controlled_file_id,archiveId,register.current_controlled_folder_id);
    const wb = new ExcelJS.Workbook();
    wb.creator="MCCS"; wb.created=new Date();
    const ws = wb.addWorksheet("Controlled VDRL");
    ws.columns = [
      ["Document Number","document_number",28],["Document Title","document_title",42],["Discipline","discipline",18],["Sub-Discipline","sub_discipline",20],["Document Type","document_type",20],["Stage","current_stage",12],["Revision","current_revision",12],["Planned Submit","planned_submit_date",16],["Actual Submit","actual_submit_date",16],["Planned Return","planned_return_date",16],["Actual Return","actual_return_date",16],["Return Code","return_code",14],["Status","current_status",22],["Days Overdue","days_overdue",14],["Responsible Party","responsible_party",18]
    ].map((x:any)=>({header:x[0],key:x[1],width:x[2]}));
    (docs??[]).forEach((d:any)=>ws.addRow(d));
    ...
    const uploaded = await uploadFileToGoogleDrive(["VDRL",supplierName,packageName,"Current"],`Controlled_VDRL_${sanitizeDriveName(supplierName)}_${stamp}.xlsx`,content as ArrayBuffer,"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    await supabase.from("vdrl_registers").update({current_controlled_file_id:uploaded.fileId,current_controlled_folder_id:currentId,last_sync_at:new Date().toISOString(),last_successful_backup_at:new Date().toISOString(),updated_at:new Date().toISOString(),updated_by:userId}).eq("id",registerId);
    if (job?.id) await supabase.from("vdrl_sync_jobs").update({status:"success",google_drive_file_id:uploaded.fileId,updated_at:new Date().toISOString()}).eq("id",job.id);
    await supabase.from("vdrl_audit_history").insert({register_id:registerId,batch_id:register.current_batch_id,user_id:userId,user_role:"super_admin",action:"Google Drive Synced",supplier_id:register.supplier_id,contractor_id:register.contractor_id,project_id:register.project_id,package_id:register.package_id,source:"MCCS Controlled Export",google_drive_file_id:uploaded.fileId});
  } catch (e:any) {
  ...
  try {
    ...
    for (const sheetName of wb.SheetNames) {
      ...
      for (let i=0;i<Math.min(rows.length,25);i++) {
        const headers=(rows[i]||[]).map(vdrlCell);
        const detected=detectVdrlMapping(headers);
        const hits=Object.keys(detected.mapping).length+(detected.mapping.document_number?5:0);
    ...
    const headers=(matrix[chosenHeader]||[]).map((x:any,i:number)=>vdrlCell(x)||`Column ${i+1}`);
    ...
    if (template?.mapping && typeof template.mapping==="object") { mapping={...mapping,...template.mapping}; confidence=Math.max(confidence,90); }
    const rawRows:any[]=[];
    ...
    try { drive=await uploadFileToGoogleDrive(["VDRL",names.supplierName,names.packageName,"Archive"],file.name,arrayBuffer,file.type||"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"); } catch(e:any){driveError=e?.message||String(e);}
  ...
  try {
    ...
    if(batch.google_drive_file_id){
      try {
        ...
        const archiveId=await ensureGoogleFolderPath(["VDRL",names.supplierName,names.packageName,"Archive"]); currentFolderId=await ensureGoogleFolderPath(["VDRL",names.supplierName,names.packageName,"Current"]);
