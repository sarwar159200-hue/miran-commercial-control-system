...
export default async function Page({
  ...
  const [vendorResult, currencyResult, projectResult, revResult, milestoneResult, itemResult, meResult, documentResult] =
    await Promise.all([
      po.vendor_id
      ...
      supabase.from("documents").select("id,document_title,document_type,document_category,file_name,revision,file_size,uploaded_at").eq("purchase_order_id",id).eq("is_deleted",false).order("uploaded_at",{ascending:false}),
    ]);
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <section className="mccs-card mt-6 rounded-2xl p-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-bold">PO Attachments</h2><p className="mt-1 text-sm text-slate-500">Files attached to this Purchase Order in MCCS / Google Drive.</p></div><Link href={`/documents/upload?po=${id}`} className="rounded-xl border bg-white px-4 py-2.5 text-sm font-bold text-blue-700">Attach another file</Link></div>
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{linkedDocuments.map((d:any)=><a key={d.id} href={`/api/documents/${d.id}/file`} target="_blank" rel="noreferrer" className="rounded-xl border p-4 transition hover:border-blue-300 hover:bg-blue-50/50"><div className="font-bold text-blue-700">{d.file_name||d.document_title||"Attached file"}</div><div className="mt-1 text-xs text-slate-500">{d.document_type||"Document"}{d.revision?` • ${d.revision}`:""}</div><div className="mt-3 text-sm font-bold">View / Download</div></a>)}{!linkedDocuments.length?<div className="rounded-xl border border-dashed p-4 text-sm text-slate-500">No PO file is attached yet.</div>:null}</div>
      </section>
