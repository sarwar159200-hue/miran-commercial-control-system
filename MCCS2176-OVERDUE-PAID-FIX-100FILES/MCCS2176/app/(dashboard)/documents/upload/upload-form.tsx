...
export function GoogleDriveUploadForm({
  ...
  const selectedInvoicePoId = useMemo(() => invoiceId ? String(invoices.find((i) => String(i.id) === invoiceId)?.purchase_order_id || "") : "", [invoices, invoiceId]);
  const effectivePoId = category === "invoice" ? selectedInvoicePoId : poId;
  const filteredMilestones = useMemo(() => {
    if (!effectivePoId) return [];
    return milestones.filter((m) => String(m.purchase_order_id || "") === effectivePoId);
  }, [milestones, effectivePoId]);
  ...
  async function uploadOneFile(file: File, values: Record<string, string | boolean>, fileIndex: number, fileCount: number) {
    ...
    // Keep chunks comfortably below serverless request limits. There is no
    // MCCS total-file-size cap: large files are sent as many resumable chunks.
    ...
    while (offset < file.size) {
      ...
      let result: any = null;
      // Retry transient network/server errors. This is particularly important
      // for large files that may spend a long time uploading over mobile links.
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          chunkResponse = await fetch("/api/google-drive/resumable/chunk", {
            method: "PUT",
            headers: {
          ...
          if (chunkResponse.status < 500 && chunkResponse.status !== 429) break;
        } catch (e) {
  ...
  return (
    <form onSubmit={submit} className="mccs-card mt-7 rounded-2xl p-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="Vendor / Contractor *"><select name="vendor_id" required className="input" value={vendorId} onChange={(e)=>{setVendorId(e.target.value);setProjectId("");setPoId("");setInvoiceId("");setMilestoneId("");}}><option value="">Select vendor</option>{vendors.map((x)=><option key={x.id} value={x.id}>{x.vendor_name}</option>)}</select></Field>
        <Field label="Project / Supply Package *"><select name="project_id" required className="input" value={projectId} onChange={(e)=>{setProjectId(e.target.value);setPoId("");setInvoiceId("");setMilestoneId("");}}><option value="">Select project / package</option>{filteredProjects.map((x)=><option key={x.id} value={x.id}>{x.project_name}{x.project_code ? ` (${x.project_code})` : ""}</option>)}</select></Field>
        <Field label="Document Section *"><select name="document_category" required className="input" value={category} onChange={(e)=>{setCategory(e.target.value);setMilestoneId("");}}><option value="po">PO</option><option value="invoice">Invoice</option><option value="srf">SRF</option><option value="other_document">Other Document</option></select></Field>
        ...
        {category === "po" ? <Field label="Purchase Order *"><select name="purchase_order_id" required className="input" value={poId} onChange={(e)=>{setPoId(e.target.value);setMilestoneId("");}}><option value="">Select PO</option>{filteredPOs.map((x)=><option key={x.id} value={x.id}>{x.po_number}</option>)}</select></Field> : <input type="hidden" name="purchase_order_id" value={poId} />}
        {category === "invoice" ? <Field label="Invoice *"><select name="invoice_id" required className="input" value={invoiceId} onChange={(e)=>{setInvoiceId(e.target.value);setMilestoneId("");}}><option value="">Select invoice</option>{filteredInvoices.map((x)=><option key={x.id} value={x.id}>{x.invoice_number}{x.po_number ? ` — ${x.po_number}` : ""}</option>)}</select></Field> : <input type="hidden" name="invoice_id" value={invoiceId} />}

        <Field label="Payment Milestone"><select name="milestone_id" className="input" value={milestoneId} onChange={(e)=>setMilestoneId(e.target.value)} disabled={!effectivePoId}><option value="">{effectivePoId ? "Not linked" : "Select a PO first"}</option>{filteredMilestones.map((x)=><option key={x.id} value={x.id}>{x.milestone_name}</option>)}</select>{effectivePoId && !filteredMilestones.length ? <span className="mt-2 block text-xs text-slate-500">No payment milestones are linked to the selected PO.</span> : null}</Field>
      ...
      <div className="mt-7 flex justify-end"><button disabled={loading || !vendorId || !projectId} className="rounded-xl bg-[#07111f] px-5 py-3 text-sm font-bold text-white disabled:opacity-50 dark:bg-blue-600">{loading ? "Uploading..." : "Upload files"}</button></div>
    </form>
  );
}
