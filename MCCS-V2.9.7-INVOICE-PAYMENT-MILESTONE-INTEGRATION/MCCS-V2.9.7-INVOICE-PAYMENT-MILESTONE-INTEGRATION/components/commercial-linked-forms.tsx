    ...
    <form action={createInvoice} className="mccs-card mt-7 rounded-2xl p-6">
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <Field label="Milestone" hint={poId ? `${poMilestones.length} milestone(s) linked to this PO.` : "Select a PO to load only its milestones."}>
          <select name="milestone_id" className="input" value={milestoneId} onChange={(e) => setMilestoneId(e.target.value)} disabled={!poId}>
        ...
        <Field label="Invoice Amount *"><input name="invoice_amount" type="number" step="0.01" required className="input" /></Field>
          ...
          <tbody className="divide-y dark:divide-slate-800">
            {rows.map((r, i) => (
              <tr key={r.id || `new-${i}`}>
                ...
                <td className="px-3 py-3">{r.id ? <span className="text-xs font-semibold text-slate-400">Existing</span> : <button type="button" className="text-sm font-bold text-red-600" onClick={() => removeNewRow(i)}>Remove</button>}</td>
              </tr>
            ))}
